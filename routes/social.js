 ...
 router.post('/facebook', async(req,res)=>{
   ...
   try{        
     ...
     try{ 
       is_exist = await axios.get(`https://graph.facebook.com/me?access_token=${fb_token}&fields=id,first_name,last_name,email,picture`);
     }catch(err){
       return res.status(400).send({msg:'token not valid', data: null , status: 400});           
     }
 ...
 // ــــــــــــــــــــG O O G L E   A U T H E N T I C A T I O N ـــــــــــــ
  ...
  router.post('/google', async(req,res)=>{
   ...
   try{        
    ...
    try{
       is_exist = await axios.get(`https://www.googleapis.com/oauth2/v3/userinfo?access_token=${google_token}`); 
    ...
    let email = is_exist.data.email? is_exist.data.email : req.body.email;
    let firstname = is_exist.data.given_name? is_exist.data.given_name : req.body.first_name;
    let lastname = is_exist.data.family_name? is_exist.data.family_name : req.body.last_name;
